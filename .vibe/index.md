...
- [`modules/viewer.md`](modules/viewer.md) — the screens that show what was loaded: the characteristics panel, and the BG element browser + composed background preview (canvas, wrapped in `web-ui-kit`'s viewport control)
...
- A canvas-drawn element irrelevant to the current classification of some other field (an `"anim"` BG element, which has no static sprite to draw) is simply not drawn at all — no placeholder, no error styling — kept visually and semantically distinct from an element whose reference genuinely fails to resolve, which does get a placeholder; the two must never share a visual treatment
