...
## Acceptance Criteria
- [x] After loading a valid stage, the panel displays its name and author
- [x] The panel displays the stage's camera bounds
- [x] The panel displays the stage's boundaries
- [x] A stage missing optional metadata (e.g. no author set) displays that explicitly (e.g. "Unknown"), not a blank or broken section

## Notes
Stage boundaries (`topBound`/`bottomBound`) are shown unconditionally, alongside an explicit statement of whether the loaded stage is 2D or 3D, derived from `bgDef.modelFile` — see `.vibe/decisions/002-stage-boundaries-shown-unconditionally-with-dimension-note.md`.
...
**Done 2026-08-26:** Implemented, tested (unit + real-browser verification against a real loaded stage), documented, and shipped.
