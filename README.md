...
<!-- vibe:begin:features -->
This project is in early-stage development. You can already load a stage by picking or dragging in the folder that contains its files — the app reads them, automatically finds the referenced background sprite sheet even in a subfolder or under a slightly different letter case, and clearly names which file is missing if it can't be found. A visible loading spinner shows the whole time the folder is being read, its sprites decoded, or its 3D model/environment loaded, so the screen never looks blank or broken while a stage loads. Once loaded, the stage's name, author, camera bounds, and stage boundaries are shown right away — a missing name or author is displayed as "Unknown" instead of a blank field, and the panel states plainly whether the stage is 2D or 3D. Every background element/layer is listed next to a live visual preview of the composed background, drawn in the correct front-to-back order — selecting an element in the list highlights it in the preview, a missing sprite shows a clear placeholder instead of breaking the preview, and a stage with no background elements shows an explicit message. For a 2D stage, the preview shows the stage's entire content by default — nothing is clipped even if it extends past the stage's declared screen area — and you can zoom and pan to explore all of it; a stage that already fits within that area looks exactly as it did before. A view toggle next to the Play/Pause control switches between this "Overview" and a "Game window" mode that shows exactly the fixed camera crop a player actually sees in-game, useful for checking the exact in-game framing — switching mid-playback never interrupts or resets it. The preview also plays back over time: a Play/Pause control starts and stops it, elements with a configured scroll ratio pan at a different rate than others, and animated background elements cycle through their frames — pausing and resuming never resets or skips ahead, and an element with broken animation data shows a clear error indicator instead of freezing or breaking the preview. Stages using Ikemen GO's 3D model extension show their actual 3D model in the preview, lit by its declared environment lighting and composited together with any flat background elements the stage also defines — freely orbit, pan, and zoom around it. A stage with no 3D model looks exactly as before, and a 3D model or lighting file that can't be found or loaded shows a clear message instead of a broken or blank preview. You can switch the app's language (English or French) live from the toolbar, with no page reload — even a currently loaded stage, an error message, or in-progress playback updates immediately and keeps running — and your choice is remembered the next time you open the app.
<!-- vibe:end:features -->
